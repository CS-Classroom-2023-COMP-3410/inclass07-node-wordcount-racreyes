...
const chalk = require('chalk'); // For coloring words
...
function readFileContent() {
    return fs.readFileSync('declaration.txt', 'utf8'); // Reads the file synchronously
}
...
function getWordCounts(content) {
    const wordCount = {};
    const words = content.toLowerCase().split(/\W+/).filter(Boolean); // Normalize to lowercase and split
...
function printColoredLines(content, wordCount) {
    ...
    for (const line of lines) {
        const coloredLine = line.split(/\W+/)
            .filter(Boolean)
            .map(word => colorWord(word, wordCount[word.toLowerCase()] || 0))
            .join(' ');
